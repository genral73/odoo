...
class TestIrAsync(TransactionCase):
    ...
    def test_model_call(self):
        job = self.env['ir.async'].call(
            self.model.echo_model,
            args=(1, 2, 3,),
            kwargs={"d": 4, "f": 6}
        )
        res = self.execute(job.id)
    ...
    def test_recursive_echo(self):
        print(self.env.uid)
        jobid = self.env['ir.async'].call(
            target=self.model.async_echo,
            kwargs={"count": 3}
        ).id
        for i in range(3):
    ...
    def test_exception(self):
        ...
        res_error = {
            ...
            'data': {
                ...
                'exception_type': res['error']['data']['exception_type']
            }
        }
    ...
    def test_recursive_error(self):
        jobid = self.env['ir.async'].call(
            target=self.model.async_cosmic_ray,
            kwargs={"count": 3}
        ).id
        for i in range(3):
            res = self.execute(jobid)
