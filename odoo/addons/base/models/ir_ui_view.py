...
* if False, the view currently does not extend its parent but can be enabled
    ...
    def _check_groups(self):
        for view in self:
            if view.type == 'qweb' and view.groups_id and view.inherit_id:
    ...
    def _accessible(self, raise_error=False):
        """ Verify is a view is accessible by the current user based on the
        groups attibute. No group specified is considered as private view.
        """
        if self.inherit_id:
            return self.inherit_id._accessible(raise_error=raise_error)
    ...
    def read_template(self, xml_id):
        ...
        self.browse(template_id)._accessible(raise_error=True)
    ...
    def render_public_asset(self, template, values=None):
        ...
        template._accessible(raise_error=True)
