...
class UserError(Exception):
    ...
    def name(self):
        warnings.warn(
            "The `name` attribute is a deprecated alias to the exception "
            "`message` attribute.",
            DeprecationWarning)
        return self.message
...
class Warning(UserError):
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "Warning is a deprecated alias to UserError and is going "
...
class RedirectWarning(Exception):
    ...
    :param str message: exception message and frontend mocal content
...
class CacheMiss(KeyError):
    ...
    def __init__(self, record, field):
        super().__init__("%s.%s" % (str(record), field.name))
