...
def apply_inheritance_specs(source, specs_tree, inherit_branding=False, pre_locate=lambda s: True):
    ...
    while len(specs):
        ...
        if node is not None:
            ...
            if pos == 'replace':
                mode = spec.get('replace_mode', 'outer')
                if mode == "outer":
                    ...
                    node_children = list(node) # Target's children
                    spec_children = list(spec) # Source's children
                ...
                # Add xpath content
                if spec.text is not None:
                    node.text = spec.text.lstrip()
                for child in spec_children:
                    node.append(child)

                # If the new content has children, right strip the tail of the last one
                new_node_children = list(node)
                if len(new_node_children) > 0:
                    node_last_children = new_node_children[-1]
                    if node_last_children.tail is not None:
                        node_last_children.tail = node_last_children.tail.rstrip()
                # Otherwise right strip the text
