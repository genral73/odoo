        ...
        $.extend($.fn.share,{
            ...
            getContent : function() {
                ...
                if($('.blog_title, .blog_content').hasClass('js_tweet')){
                    ...
                    var text = btoa(_.str.sprintf(tweet, selectedText, window.location.href));
                    popover_content += _.str.sprintf(
                        "<a onclick=\"window.open('%s' + encodeURI(atob('%s')), '_%s','location=yes,height=570,width=520,scrollbars=yes,status=yes')\"><i class=\"ml4 mr4 fa fa-twitter fa-lg\"/></a>",
