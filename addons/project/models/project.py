...
class Task(models.Model):
    ...
    def _message_post_after_hook(self, message, msg_vals):
        if message.attachment_ids:
            image_count = self.env['ir.attachment'].search_count([
                ('id', 'not in', message.attachment_ids.ids),
                ('res_model', '=', 'project.task'),
                ('res_id', '=', self.id),
                ('mimetype', 'ilike', 'image')
            ])
            if not image_count:
                attached_image = message.attachment_ids.search([('mimetype', 'ilike', 'image')], limit=1)
                self.displayed_image_id = attached_image
