...
class StockRule(models.Model):
    ...
    def _run_buy(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            supplier = False
            if procurement.values.get('supplier_id') and procurement.values['supplier_id']._name == 'product.supplierinfo':
                supplier = procurement.values['supplier_id']
            else:
    ...
    def _get_lead_days(self, product):
        ...
        if supplier_delay:
            delay_description += '<tr><td>%s</td><td class="float-right">+ %d %s</td></tr>' % (_('Vendor Lead Time'), supplier_delay, _('day(s)'))
        security_delay = buy_rule.picking_type_id.company_id.po_lead
        delay_description += '<tr><td>%s</td><td class="float-right">+ %d %s</td></tr>' % (_('Purchase Security Lead Time'), security_delay, _('day(s)'))
        days_to_purchase = buy_rule.company_id.days_to_purchase
        delay_description += '<tr><td>%s</td><td class="float-right">+ %d %s</td></tr>' % (_('Days to Purchase'), days_to_purchase, _('day(s)'))
