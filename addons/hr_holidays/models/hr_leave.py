...
class HolidaysRequest(models.Model):
    ...
    state = fields.Selection([
        ...
        ('validate', 'Approved')
        ], string='Status', compute='_compute_state', store=True, readonly=True, tracking=True, copy=False, default='draft',
        ...
        for holiday in self:
            if holiday.holiday_type == 'employee':
                if not holiday.employee_id:
                    holiday.employee_id = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1).id
        ...
        for holiday in self:
            if holiday.employee_id:
                ...
                if not holiday.department_id:
                    holiday.department_id = self.env['hr.employee'].search([('user_id', '=', self.env.uid)], limit=1).department_id.id
            else:
