...
class AdyenTerminal(models.Model):
    ...
    def _sync_adyen_terminals(self):
        for adyen_store_id in self.env['adyen.store'].search([]):
            ...
            # Added terminals
            for terminal in set(response['uniqueTerminalIds']) - terminals_in_db:
                self.sudo().create({
            ...
            # Removed terminals
            for terminal in terminals_in_db - set(response['uniqueTerminalIds']):
                self.search([('terminal_uuid', '=', terminal)]).unlink()
