...
class PosPaymentMethod(models.Model):
    ...
    def create(self, vals):
        payment_method = super(PosPaymentMethod, self).create(vals)
        if payment_method.use_payment_terminal == 'odoo_adyen' and 'adyen_terminal_id' in vals:
    ...
    def write(self, vals):
        ...
        if self.use_payment_terminal == 'odoo_adyen' and 'adyen_terminal_id' in vals:
            self.adyen_terminal_identifier = self.adyen_terminal_id.terminal_uuid
        return res
    ...
    def proxy_adyen_request(self, data):
        ...
        if self.use_payment_terminal == 'odoo_adyen':
            return self._proxy_adyen_request_odoo_proxy(data)
        else:
            return self._proxy_adyen_request_direct(data)

    def _proxy_adyen_request_direct(self, data):
        self.ensure_one()
        TIMEOUT = 10
    ...
    def _proxy_adyen_request_odoo_proxy(self, data):
        try:
            return self.env.company.adyen_account_id._adyen_rpc('terminal_request', {
                'request_data': data,
                'account_code': self.adyen_payout_id.code,
