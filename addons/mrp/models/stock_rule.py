...
class StockRule(models.Model):
    ...
    def _get_lead_days(self, product):
        ...
        if manufacture_delay:
            ...
            delay_description += '<tr><td>%s</td><td class="float-right">+ %d %s</td></tr>' % (_('Manufacturing Lead Time'), manufacture_delay, _('day(s)'))
        security_delay = manufacture_rule.picking_type_id.company_id.manufacturing_lead
        if security_delay:
            delay += security_delay
            delay_description += '<tr><td>%s</td><td class="float-right">+ %d %s</td></tr>' % (_('Company Manufacture Lead Time'), security_delay, _('day(s)'))
        return delay, delay_description
