    ...
    def generate_email(self, res_ids, fields):
        ...
        for record in self.env[self.model].browse(res_ids):
            if record.l10n_ch_isr_valid:
                ...
                report_name = 'ISR-' + self._render_field('report_name', record.ids, set_lang=True)[record.id] + '.pdf'

                pdf = self.env.ref('l10n_ch.l10n_ch_isr_report').render_qweb_pdf(record.ids)[0]
                pdf = base64.b64encode(pdf)

                attachments_list = multi_mode and result[record.id].get('attachments', False) or result.get('attachments', False)
                if attachments_list:
                    attachments_list.append((report_name, pdf))
                else:
                    result[record.id]['attachments'] = [(report_name, pdf)]
        return result
