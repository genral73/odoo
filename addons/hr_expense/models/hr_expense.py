...
class HrExpense(models.Model):
    ...
    name = fields.Char('Description', compute='_compute_from_product_id_company_id', store=True, readonly=True, required=True, copy=True,
    ...
    employee_id = fields.Many2one('hr.employee', compute='_compute_employee_id', store=True, string="Employee", required=True, readonly=False, tracking=True, copy=True,
    ...
    product_uom_id = fields.Many2one('uom.uom', string='Unit of Measure', compute='_compute_from_product_id_company_id', store=True, readonly=True, copy=True,
        states={'draft': [('readonly', False)], 'refused': [('readonly', False)]}, default=_default_product_uom_id, domain="[('category_id', '=', product_uom_category_id)]")
    product_uom_category_id = fields.Many2one(related='product_id.uom_id.category_id', readonly=True)
    unit_amount = fields.Float("Unit Price", compute='_compute_from_product_id_company_id', store=True, readonly=True, required=True, copy=True,
    ...
    tax_ids = fields.Many2many('account.tax', 'expense_tax', 'expense_id', 'tax_id', compute='_compute_from_product_id_company_id', store=True, readonly=False, copy=True,
    ...
    def _compute_from_product_id_company_id(self):
        for expense in self:
            if expense.product_id:
                ...
                account = expense.product_id.product_tmpl_id.with_company(expense.company_id)._get_product_accounts()['expense']
                if account:
...
class HrExpenseSheet(models.Model):
    ...
    department_id = fields.Many2one('hr.department', compute='_compute_from_employee_id', store=True, readonly=False, string='Department', states={'post': [('readonly', True)], 'done': [('readonly', True)]})
