...
class ResConfigSettings(models.TransientModel):
    ...
    expense_alias_prefix = fields.Char('Default Alias Name for Expenses', compute='_compute_expense_alias_prefix', store=True, readonly=False)
    ...
    def _compute_expense_alias_prefix(self):
        for settings in self:
            if not settings.use_mailgateway:
                settings.expense_alias_prefix = False
