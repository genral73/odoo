...
QUnit.test('activity view: no group_by_menu and no time_range_menu', async function (assert) {
    ...
    assert.isVisible(actionManager.$('.o_search_options .o_dropdown.o_filters_menu > button'),
        "filter should be available in view search");
    assert.isVisible(actionManager.$('.o_search_options .o_dropdown.o_favorites_menu > button'),
