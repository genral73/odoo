...
class MailTemplate(models.Model):
    ...
    def _render_template_jinja(self, template_txt, model, res_ids):
        ...
        * ``object``: record of the document record this mail is related to
        ...
        if any(r is None for r in res_ids):
            print('MEGACACABOOM WITH RES IDS', self, model, res_ids)
        resid_to_rec = dict((record.id, record) for record in self.env[model].browse(res_ids))
    ...
    def _render_template(self, template_txt, model, res_ids, engine='jinja', post_process=False):
        ...
        :param boolean post_process: perform html post processing
        """
        if not isinstance(res_ids, (list, tuple)):
            raise ValueError(_('Template rendering should be called only using on a list of IDS.'))
        if engine != 'jinja':
            raise ValueError(_('Template rendering supported only jinja.'))

        rendered = self._render_template_jinja(template_txt, model, res_ids)
        if post_process:
            for res_id, result in rendered.items():
                rendered[res_id] = self._render_template_postprocess_html(result)

        return rendered

    def _get_lang_context(self, res_ids):
        """ Given some record ids, return the lang for each record based on
        lang field of template.
        ...
        if res_ids is None:
            return {None: self}
        if not isinstance(res_ids, (list, tuple)):
            raise ValueError(_('Template rendering methods should be called with an ID list or None if no record is targeted.'))
    ...
    def _classify_per_lang(self, res_ids):
        """ Given some record ids, return for computed lang, a contextualized
        template and its ids subset of res_ids.

        :return dict: template with lang=lang_code if specific land computed
          or template: res_ids concerned by that language """
        self.ensure_one()

        lang_to_res_ids = {}
        for res_id, lang in self._get_lang_context(res_ids).items():
            lang_to_res_ids.setdefault(lang, []).append(res_id)

        return dict(
            (self.with_context(lang=lang) if lang else self, lang_res_ids)
            for lang, lang_res_ids in lang_to_res_ids.items()
        )

    def _render_field(self, field, res_ids, set_lang=False, post_process=False):
        """ Tool method: render a given field of template on the set of record
        ids. """
        self.ensure_one()
        if set_lang:
            templates_res_ids = self._classify_per_lang(res_ids)
        else:
            templates_res_ids = {self: res_ids}

        return dict(
            (res_id, rendered)
            for template, tpl_res_ids in templates_res_ids.items()
            for res_id, rendered in template._render_template(template[field], template.model, tpl_res_ids, post_process=post_process).items()
        )
    ...
    def generate_email(self, res_ids, fields):
        ...
        results = dict()
        for template, template_res_ids in self._classify_per_lang(res_ids).items():
            for field in fields:
