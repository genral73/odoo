...
var LunchListRenderer = ListRenderer.extend({
    ...
    _renderRow: function (record) {
        ...
        tr.attr('data-product-id', record.data.product_id ? record.data.product_id.res_id : record.data.id);
        return tr;
    },
    ...
    _onClickListRow: function (ev) {
        ...
        if (!($(ev.target).hasClass('o_list_record_selector') || $(ev.target).hasClass('custom-control') || $(ev.target).hasClass('custom-control-input') || $(ev.target).hasClass('custom-control-label'))) {
            ev.preventDefault();
