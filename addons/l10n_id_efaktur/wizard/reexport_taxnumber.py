...
class ReexportTaxnumber(models.TransientModel):
    ...
    start_number = fields.Char('From', compute='_compute_start_number', store=True, readonly=False)
    ...
    @api.depends('last_available')
    def _compute_start_number(self):
        self.start_number = "0000000000000"
        for record in self:
            if record.last_available:
                self.start_number = '%013d' % (int(record.last_available.name) + 1)
