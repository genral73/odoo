...
class EventTypeMail(models.Model):
    ...
    trigger_stage_id = fields.Many2one(
        'event.stage', string='Stage', readonly=False, store=True, copy=True, compute='_compute_trigger_stage_id',
    ...
    def _compute_trigger_stage_id(self):
        for mail in self:
            if mail.trigger_stage_id and mail.interval_type != 'stage_update':
...
class EventMailScheduler(models.Model):
    ...
    trigger_stage_id = fields.Many2one(
        'event.stage', string='Stage', readonly=False, store=True, copy=True, compute='_compute_trigger_stage_id',
        help='Used when the interval type is "On stage update". When the event is moved into this stage, the event mail will be triggered.')
    trigger_stage_date = fields.Datetime('Stage change date', help='Date when the event was moved to the sage')
    template_id = fields.Many2one(
        'mail.template', string='Email Template',
        domain=[('model', '=', 'event.registration')], ondelete='restrict',
        help='This field contains the template of the mail that will be automatically sent')
    ...
    @api.depends('done', 'interval_type', 'scheduled_date', 'mail_sent')
