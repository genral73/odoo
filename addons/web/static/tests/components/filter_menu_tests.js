odoo.define('web.filter_menu_tests', function (require) {
    ...
    QUnit.module('FilterMenu', {
        ...
        QUnit.test('should have Date and ID field proposed in that order in "Add custom Filter" submenu', async function (assert) {
            ...
            const optionEls = el.querySelectorAll('div.o_filter_condition > select.o_generator_menu_field option');
        ...
        QUnit.test('toggle a "simple" filter in filter menu works', async function (assert) {
            ...
            assert.ok(helpers.isItemSelected("Foo"));

            await helpers.toggleMenuItem("Foo");
            assert.deepEqual(helpers.getFacetTexts(), []);
            assert.notOk(helpers.isItemSelected("Foo"));
        ...
        QUnit.test('add a custom filter works', async function (assert) {
            ...
            await testUtils.fields.editSelect(el.querySelector('div.o_filter_condition > select.o_generator_menu_field'), 'id');
            await testUtils.fields.editInput(el.querySelector('div.o_filter_condition > span.o_generator_menu_value > input'), 1);

            await helpers.applyFilter();

            assert.deepEqual(helpers.getFacetTexts(), ['ID is "1"']);
        ...
        QUnit.test('deactivate a new custom filter works', async function (assert) {
            ...
            assert.ok(helpers.isItemSelected('Date is equal to "02/05/2020"'));
            assert.deepEqual(helpers.getFacetTexts(), ['Date is equal to "02/05/2020"']);

            await helpers.toggleMenuItem('Date is equal to "02/05/2020"');

            assert.notOk(helpers.isItemSelected('Date is equal to "02/05/2020"'));
            assert.deepEqual(helpers.getFacetTexts(), []);
        ...
        QUnit.test('filter by a date field using period works', async function (assert) {
            ...
            assert.ok(helpers.isOptionSelected("Date", 0));
        ...
        QUnit.test('filter by a date field using period works even in January', async function (assert) {
            ...
            assert.deepEqual(helpers.getFacetTexts(), ["Date: December 2016"]);

            await helpers.toggleFilterMenu();
            await helpers.toggleMenuItem("Date");

            assert.ok(helpers.isItemSelected("Date"));
            assert.ok(helpers.isOptionSelected("Date", 'December'));
            assert.ok(helpers.isOptionSelected("Date", '2016'));
        ...
        QUnit.test('filter with date attribute set as search_default', async function (assert) {
            ...
            assert.deepEqual(helpers.getFacetTexts(), ["Date: June 2019"]);
        ...
        QUnit.test('arch order of groups of filters preserved', async function (assert) {
            ...
            await helpers.toggleFilterMenu();
            assert.containsN(el, '.o_filters_menu .o_menu_item', 11);

            const menuItemEls = el.querySelectorAll('.o_filters_menu .o_menu_item');
