odoo.define('web.dropdown_menu_tests', function (require) {
    ...
    async function createDropdownMenu({ items, onItemSelected, debug }) {
        class Parent extends owl.Component {
            constructor() {
                super();
                this.state = owl.hooks.useState({ items, title: "Dropdown" });
            }
            _onItemSelected(ev) {
                if (onItemSelected) {
                    onItemSelected.bind(this)(ev);
                }
            }
        }
        Parent.components = { DropdownMenu };
        Parent.env = makeTestEnvironment();
        Parent.template = owl.tags.xml`<DropdownMenu t-props="state" t-on-item-selected="_onItemSelected"/>`;
    ...
    QUnit.module('Components', {
            ...
            this.items = [
                {
                    ...
                    groupNumber: 1,
                    options: [
                        { description: "First Option", groupNumber: 1, optionId: 1 },
                        { description: "Second Option", groupNumber: 2, optionId: 2 },
                    ],
                },
                {
                    isActive: true,
        ...
        QUnit.test('simple rendering and basic interactions', async function (assert) {
            ...
            const { parent, el } = await createDropdownMenu({
                items: this.items,
            });

            assert.strictEqual(el.querySelector('button').innerText, "Dropdown");
            assert.containsNone(el, 'ul.o_dropdown_menu');

            await testUtils.dom.click(el.querySelector('button'));

            assert.containsN(el, '.dropdown-divider, .dropdown-item', 3,
                'should have 3 elements counting the divider');
            const itemEls = el.querySelectorAll('.o_menu_item > .dropdown-item');
            assert.strictEqual(itemEls[0].innerText, 'Some Item');
            assert.doesNotHaveClass(itemEls[0], 'selected');
            assert.hasClass(itemEls[1], 'selected');

            const dropdownElements = el.querySelectorAll('.o_menu_item *');
            for (const dropdownEl of dropdownElements) {
                await testUtils.dom.triggerEvents(dropdownEl, ['mousedown', 'click']);
            }
            assert.containsOnce(el, 'ul.o_dropdown_menu',
                "Clicking on any item of the dropdown should not close it");

            await testUtils.dom.triggerEvents(document.body, ['mousedown', 'click']);
        ...
        QUnit.test('click on an item without options should toggle it', async function (assert) {
            ...
            const firstItemEl = el.querySelector('.o_menu_item > a');
        ...
        QUnit.test('options rendering', async function (assert) {
            ...
            assert.containsN(el, '.dropdown-divider, .dropdown-item', 6);
        ...
        QUnit.test('click on an option should trigger the event "item_option_clicked" with appropriate data', async function (assert) {
            ...
            // open dropdown menu
            await testUtils.dom.click(el.querySelector('button'));
            assert.containsN(el, '.dropdown-divider, .o_menu_item', 3);

            // open menu options of first item
            await testUtils.dom.click(el.querySelector('.o_menu_item > a'));
            let optionELs = el.querySelectorAll('.o_menu_item  a.o_item_option');

            // click on first option
            await testUtils.dom.click(optionELs[0]);
            assert.hasClass(el.querySelector('.o_menu_item > a'), 'selected');
            optionELs = el.querySelectorAll('.o_menu_item  a.o_item_option');
            ...
            await testUtils.dom.click(optionELs[1]);
            assert.doesNotHaveClass(el.querySelector('.o_menu_item > a'), 'selected');
            optionELs = el.querySelectorAll('.o_menu_item  a.o_item_option');
        ...
        QUnit.test('keyboard navigation', async function (assert) {
            ...
            assert.isActive(el.querySelector('.o_menu_item a'));
            assert.containsNone(el, '.o_item_option');

            await navigate('ArrowRight'); // Unfold first item's options (w/ Right)

            assert.isActive(el.querySelector('.o_menu_item a'));
            assert.containsN(el, '.o_item_option', 2);

            await navigate('ArrowDown'); // Go to next option

            assert.isActive(el.querySelector('a.o_item_option'));
            ...
            assert.containsNone(el, 'ul.o_dropdown_menu', "Dropdown should be folded");
