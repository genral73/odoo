odoo.define('web.favorite_menu_tests', function (require) {
    ...
    const { createControlPanel, mock, createView } = testUtils;
    ...
    QUnit.module('FavoriteMenu', {
        ...
        QUnit.test('simple rendering with no favorite', async function (assert) {
            ...
            assert.containsOnce(el, 'div.o_favorites_menu > button i.fa.fa-star');
            assert.strictEqual(el.querySelector('div.o_favorites_menu > button span').innerText.trim(), "Favorites");

            await helpers.toggleFavoriteMenu();
            assert.containsNone(el, '.dropdown-divider');
            assert.containsOnce(el, '.o_add_favorite');
            assert.strictEqual(el.querySelector('.o_add_favorite > button').innerText.trim(),
                "Save Current Search");

            await helpers.toggleSaveFavorite();
            assert.strictEqual(
        ...
        QUnit.test('delete an active favorite remove it both in list of favorite and in search bar', async function (assert) {
            ...
            assert.deepEqual(helpers.getFacetTexts(), ['My favorite']);
            assert.hasClass(el.querySelector('.o_favorites_menu .o_menu_item > a'), 'selected');
            ...
            assert.deepEqual(helpers.getFacetTexts(), []);
            await helpers.toggleFavoriteMenu();
            const itemEls = el.querySelectorAll('.o_favorites_menu .o_menu_item');
