odoo.define('web.test_utils_create_control_panel', function (require) {
    ...
    const helpers = {
        ...
        async toggleMenu({ el, rootEl }, menuFinder) {
            const menu = findItem(el, `${rootEl} .o_dropdown > button`, menuFinder);
            await click(menu);
        },
        ...
        async toggleFilterMenu({ el, rootEl }) {
            await click(el.querySelector(`${rootEl} .o_filter_menu button`));
        },
        ...
        async toggleGroupByMenu({ el, rootEl }) {
            await click(el.querySelector(`${rootEl} .o_group_by_menu button`));
        },
        ...
        async toggleFavoriteMenu({ el, rootEl }) {
            await click(el.querySelector(`${rootEl} .o_favorite_menu button`));
        },
        async toggleSaveFavorite({ el, rootEl }) {
            await click(el.querySelector(`${rootEl} .o_favorite_menu .o_add_favorite button`));
        },
        async editFavoriteName({ el, rootEl }, name) {
            await editInput(el.querySelector(`${rootEl} .o_favorite_menu .o_add_favorite input[type="text"]`), name);
        },
        async saveFavorite({ el, rootEl }) {
            await click(el.querySelector(`${rootEl} .o_favorite_menu .o_add_favorite button.o_save_favorite`));
        },
        async deleteFavorite({ el, rootEl }, favoriteFinder) {
            const favorite = findItem(el, `${rootEl} .o_favorite_menu .o_menu_item`, favoriteFinder);
    ...
    /**
     * Binds a suite of helper functions used to interact with components related
     * to the Control Panel. Parameters are the element in which to find the subcomponents
     * and an optional "root" selector string used to select elements with more
     * precision.
     * @param {Element} el
     * @param {string} root
     * @returns {Function[]} see the object up above for a list of available control
     *      panel helpers
     */
