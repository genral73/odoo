...
var AbstractController = mvc.Controller.extend(ActionMixin, WidgetAdapterMixin, {
    ...
    start: async function () {
        ...
        if (this.withControlPanel) {
            if (this.$buttons) {
                this.controlPanelProps.buttons = this.$buttons;
            }
            Object.assign(this.controlPanelProps, {
                pager: this._getPagerProps(this.initialState),
                sidebar: this._getSidebarProps(this.initialState),
                title: this.getTitle(),
            });
        ...
        await parentPromise;
        await this._update(this.initialState, { updateControlPanel: false });
    },
    /**
     * @override
     *
    ...
    on_attach_callback: function () {
        ...
        if (this.withControlPanel) {
            this._controlPanelStore.on('get-controller-query-params', this, this._onGetOwnedQueryParams);
        }
    ...
    on_detach_callback: function () {
        ...
        if (this.withControlPanel) {
            this._controlPanelStore.off('get-controller-query-params', this);
        }
    ...
    update: function (params, options) {
        ...
        return this.dp.add(def).then(function (handle) {
            ...
            return self.dp.add(self.updateRendererState(state, params)).then(function () {
                ...
                params.updateControlPanel = false;
     ...
     * @param {Object} [params={}]
     * @param {Object} [params.updateControlPanel=true]
     * @returns {Promise}
     */
    _update: async function (state, params = { updateControlPanel: true }) {
        ...
        this._updateButtons();
        if (this.withControlPanel && params.updateControlPanel) {
            const newProps = {
