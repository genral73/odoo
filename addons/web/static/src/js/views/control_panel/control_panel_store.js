odoo.define('web.ControlPanelStore', function (require) {
    ...
    class ControlPanelStore extends Store {
        ...
        async editFavorite({ state }, filterId, values) {
            ...
            if (
                Object.keys(irFilter).some(k => ['context', 'domain', 'sort'].includes(k)) ||
                this.modelName === 'ir.filters' // update the view anyway if the filter itself is visible
            ) {
                updateQuery = true;
            }
            if ('userId' in values) {
            ...
            await this.env.dataManager.edit_filter(irFilter);
        }
         ...
         * @todo
         */
        addAutoCompletionValues({ state }, { filterId, label, value, operator }) {
            const activity = state.query.some(queryElem =>
            ...
            if (!activity) {
                ...
                state.query.push({ filterId, groupId, label, value, operator });
            }
        }
        ...
        toggleFilter({ state }, filterId) {
            ...
            if (index >= 0) {
                ...
                const { groupId, type } = state.filters[filterId];
                if (type === 'favorite') {
                    state.query = [];
                }
                state.query.push({ groupId, filterId });
        ...
        getFiltersOfType({ state }, type) {
            const fs = Object.values(state.filters).reduce(
                (filters, filter) => {
        ...
        getQuery() {
            ...
            console.log(`Ordered by:`, query.orderedBy && query.orderedBy.map(o => o.name));
            if (this.searchMenuTypes.includes('timeRange')) {
            ...
            defaultFilters
                ...
                .forEach(f => {
                    if (f.hasOptions) {
                        ...
                        this.dispatch('addAutoCompletionValues', { filterId: f.id, value, operator, label });
                    } else {
        ...
        _getContext(groups) {
            ...
            const filterContexts = groups.reduce(
                (contexts, group) => {
        ...
        */
                ...
                (orderedBy, group) => {
                    if (group.type === 'favorite') {
                        ...
                        if (favoriteOrderedBy) {
                            ...
                            return [...favoriteOrderedBy, ...orderedBy];
                        }
                    }
                    return orderedBy;
                },
        ...
        async _saveQuery(preFilter) {
            ...
            let controllerQueryParams = await new Promise(resolve => {
                this.trigger('get-controller-query-params', resolve);
            });
        ...
        _setProperties(config) {
            ...
            this.intervalOptions = Object.values(INTERVAL_OPTIONS).map(
