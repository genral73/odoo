odoo.define('web.ControlPanelModel', function (require) {
     ...
     *      - fieldName: string
     *      - fieldType: string
     *      - groupNumber: number, used to separate items in the 'Group by' menu
     *      - [isDefault]: boolean
     *          if isDefault = true:
     ...
     *
     * - type 'field':
     *      - label: string, description put in the facet (can be temporarilly missing)
    ...
    class ControlPanelModel extends Model {
        ...
        toggleFilterWithOptions(filterId, optionId) {
            ...
            if (index >= 0) {
                ...
                if (filter.type === 'filter') {
                    ...
                    this._insert({ groupId: filter.groupId, filterId, optionId });
                }
            }
        }
        ...
        */
        ...
        /**
         * Insert a queryElement { groupId, filterId, optionId } into (this.state.)query for
         * a corresponding filter of type 'groupBy' with options.
         * The array query is by construction of the form
         *
         * A1 'concat' A2 'concat' A3
         *
         * where filterId does not occur in the arrays A1 and A3 (possibly empty) and
         * A2 = [{ groupId, filterId, optionId_1}, ..., { groupId, filterId, optionId_n}]
         * (possibly empty).
         *
         * The idea of the function below is to insert the queryElement at the end
         * of query if A2 is empty and inside A2 otherwise while keeping A2 ordered
         * for the natural order on option ids: 'year' < 'quarter' < ... < 'day'.
         *
         * @private
         * @param {Objec} queryElem
         */
        _insert(queryElem) {
            const { filterId } = queryElem;
            let firstIndex = -1;
            let lastIndex = -1;
            this.state.query.forEach((qe, i) => {
                if (qe.filterId === filterId) {
                    firstIndex = firstIndex === -1 ? i : firstIndex;
                    lastIndex = i;
                }
            });
            if (firstIndex === -1) { // case A2 empty
                this.state.query.push(queryElem);
                return;
            }
            // case A2 non empty
            const a1 = this.state.query.slice(0, firstIndex);
            const a2 = this.state.query.slice(firstIndex, lastIndex + 1);
            const a3 = this.state.query.slice(lastIndex + 1);

            a2.push(queryElem);
            a2.sort((qe1, qe2) => rankInterval(qe1.optionId) - rankInterval(qe2.optionId));

            this.state.query = [].concat(a1, a2, a3);
        }
