...
class GoogleCalendarService():
    ...
    def get_events(self, sync_token=None, token=None, timeout=TIMEOUT):
        ...
        try:
            status, data, time = self.google_service._do_request(url, params, headers, type='GET', timeout=timeout)
    ...
    def delete(self, event_id, token=None, timeout=TIMEOUT):
        ...
        params = {'access_token': token}
        try:
            self.google_service._do_request(url, params, headers=headers, type='DELETE', timeout=timeout)
        except requests.HTTPError as e:
