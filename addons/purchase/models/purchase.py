...
class PurchaseOrder(models.Model):
    ...
    def retrieve_dashboard(self):
        ...
        # Calculated values ('order value' and 'days to purchase') note that 'order value' takes into account exchange rate
        # This is done via SQL for scalability reasons

        # For all values
        query = """SELECT AVG(COALESCE(amount_total / NULLIF(currency_rate, 0), amount_total)),
                          AVG(extract(epoch from age(date_approve,create_date)/(24*60*60)::decimal(16,2))),
                          SUM(CASE WHEN date_approve >= %s THEN COALESCE(amount_total / NULLIF(currency_rate, 0), amount_total) ELSE 0 END)
                   FROM purchase_order
        ...
        res = self.env.cr.fetchone()
        result['all_avg_order_value'] = round(res[0] or 0, 2)
        result['all_avg_days_to_purchase'] = round(res[1] or 0, 2)
        result['all_total_last_7_days'] = round(res[2] or 0, 2)
