...
class Survey(models.Model):
    ...
    def _get_default_session_code(self):
        # AWA check me: might be over-engineered
        # maybe would be enough to start at 0001 and increment...
        # but it' a bit weird/sad to ask your attendees to type "0001"
        """ Attempt to generate a session code for our survey.
        The method will first try to generate 10 codes with 4 digits each and check if any are colliding.
        If we have at least one non-colliding code, we use it.
        If all 10 generated codes are colliding, we try with 10 codes of 5 digits,
        ...
        for digits_count in range(4, 10):
            ...
            code_candidates = set([str(random.randint(range_lower_bound, range_upper_bound)) for i in range(10)])
