...
class SurveyInvite(models.TransientModel):
    ...
    # composer content
    subject = fields.Char('Subject', compute='_compute_template_values', store=True, readonly=False)
    body = fields.Html('Contents', default='', sanitize_style=True, compute='_compute_template_values', store=True, readonly=False)
    ...
    def _compute_template_values(self):
        for invite in self:
            if invite.subject is None:
                invite.subject = False
            if invite.body is None:
                invite.body = False
