...
class AccountInvoiceReport(models.Model):
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        result = super(AccountInvoiceReport, self).read_group(domain, fields, set(groupby) | set(['currency_id']), offset, limit, orderby, lazy)
        for res in result:
            if self.env.company.currency_id.id != res['currency_id'][0]:
                for field in set(['amount_total', 'price_average', 'price_subtotal', 'residual']) & set(res):
