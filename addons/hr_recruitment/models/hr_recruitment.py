...
class Applicant(models.Model):
    ...
    email_from = fields.Char("Email", size=128, help="Applicant email", compute='_compute_partner_phone_email', inverse='_inverse_partner_email', store=True, readonly=False)
    ...
    partner_phone = fields.Char("Phone", size=32, compute='_compute_partner_phone_email', inverse='_inverse_partner_phone', store=True, readonly=False)
    partner_mobile = fields.Char("Mobile", size=32, compute='_compute_partner_phone_email', inverse='_inverse_partner_mobile', store=True, readonly=False)
    ...
    def _inverse_partner_email(self):
        for applicant in self:
            if applicant.partner_id and applicant.email_from and not applicant.partner_id.email:
                applicant.partner_id.email = applicant.email_from

    def _inverse_partner_phone(self):
        for applicant in self:
            if applicant.partner_id and applicant.partner_phone and not applicant.partner_id.phone:
                applicant.partner_id.phone = applicant.partner_phone

    def _inverse_partner_mobile(self):
        for applicant in self:
            if applicant.partner_id and applicant.partner_mobile and not applicant.partner_id.mobile:
